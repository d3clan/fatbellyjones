package com.viviquity.readmy.facebook;
...
public class FacebookUserFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException,
	    ServletException {
	try {
	    MDC.put(IP_ADDRESS_KEY, req.getRemoteAddr());
	    ...
	    if (userClient == null) {
		logger.debug("User has no Facebook client. Creating and storing to user's session.");
	    ...
	    MDC.put(FACEBOOK_ID_KEY, String.valueOf(facebookUserID));

	    chain.doFilter(request, response);
	} finally {
	    MDC.remove(IP_ADDRESS_KEY);
	    MDC.remove(FACEBOOK_ID_KEY);
	}
    }
